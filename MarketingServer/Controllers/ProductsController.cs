...
namespace MarketingServer
{
    public class ProductsController : ApiController
    {
        ...
        public async Task<IHttpActionResult> GetProducts(string customerId)
        {
            ...
            //List<Vendor> vendors = await db.Vendors.ToListAsync();

            //foreach(Vendor vendor in vendors)
            //{
                
            //    HttpWebRequest request = (HttpWebRequest)WebRequest.Create("https://api.clickbank.com/rest/1.3/products/list?site=" + vendor.Name);
            //    request.Accept = "application/json";
            //    request.Headers.Add(HttpRequestHeader.Authorization, "DEV-SHE17V09PLJ3MARVSI502TNSEULV7U09:" + vendor.APIKey);
            //    request.Method = "GET";
            ...
            //    string prods = sb.ToString();

            //    JObject jObject = JObject.Parse(prods);

            //    var cbProducts = jObject.SelectToken("products.product").Select(x => new { sku = (string)x["@sku"], price = x["pricings"]["pricing"]["standard"]["price"]["native_price"] }).ToArray();



            //    List<Product> prducts = await db.Products.Where(x => x.VendorID == vendor.ID).ToListAsync();

            //    foreach(Product product in prducts)
            //    {
            //        double cbPrice = (double)cbProducts.Where(x => x.sku == product.Sku).Select(x => x.price).FirstOrDefault();

            //        if(product.Price != cbPrice)
            //        {
            //            product.Price = cbPrice;
            //        }
            //    }
            //}
